...
def extend_stress_period_data(stress_period_data):  # the dictionary version
    ...
    for index in range(180):
        ...
        if start_sp <= index <= end_sp:
            new_spd[sp_match_dict[index]] = stress_period
...
def extend_time(pd_, new_sp_):
    for sp in range(4*12):
        pd_.append((30.4, 5, 1.0))
    for sp in range(17*4):
...
def extend_rch(stress_period_data):
    ...
    for index in range(180):
        ...
        if start_sp <= index <= end_sp:
            new_irch[sp_match_dict[index]] = {'filename': os.path.join('..', 'external_files', 'arrays', 'irch.asc')}
            new_spd[sp_match_dict[index]] = {'filename': os.path.join('..', 'external_files', 'arrays', 'rch', rch_file), 'data': stress_period}
...
new_sp = 17 * 4 + 4 * 12  # 17 years with quarterly stress periods, 4 years with monthly stress periods
start_sp = 112  # 1999
end_sp = start_sp + new_sp  # 2019

sp_match_dict = {}
for sp in range(start_sp, end_sp):
    sp_match_dict[sp] = 180 + (new_sp - (sp - start_sp))

print('\nmaking directories...')
new_path = os.path.join('EXT_2019_1999')
